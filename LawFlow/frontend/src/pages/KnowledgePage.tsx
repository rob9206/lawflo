...
export default function KnowledgePage() {
  ...
  return (
    <div>
      ...
      {isLoading ? (
        <div className="text-ui-muted">Searching...</div>
      ) : chunks.length === 0 ? (
        <EmptyState
          ...
          {chunks.map((chunk) => (
            <Card key={chunk.id}>
              <div className="flex items-center gap-2 mb-2">
                <Badge variant="accent">{chunk.subject}</Badge>
              ...
              {chunk.summary && (
                <p className="text-sm mb-2 text-ui-secondary">{chunk.summary}</p>
              )}
              <p className="text-xs line-clamp-3 text-ui-muted">{chunk.content}</p>
